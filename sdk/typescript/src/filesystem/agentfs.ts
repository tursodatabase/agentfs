    ...
    private async writeDataAtOffset(offset: number, data: Buffer): Promise<void> {
        ...
        for (let chunkIdx = startChunk; chunkIdx <= endChunk; chunkIdx++) {
            ...
            const upsertStmt = this.db.prepare(`
                ...
                VALUES (?, ?, ?) ON CONFLICT(ino, chunk_index) DO
...
export class AgentFS implements FileSystem {
    ...
    private async ensureMaxBytes(): Promise<void> {
        const stmt = this.db.prepare("SELECT value from fs_config WHERE key = 'max_bytes'")
        const row = await stmt.get() as { value: string } | undefined

        if (!row) {
            const insert = this.db.prepare(`
                INSERT INTO fs_config(key, value)
                VALUES ('max_bytes', '0')
            `)
    ...
    private async getMaxBytes(): Promise<number | null> {
        const stmt = this.db.prepare("SELECT value FROM fs_config WHERE key = 'max_bytes'")
        const row = await stmt.get() as { value: string } | undefined
        ...
        const bytes = Number.parseInt(row.value, 10)
    ...
    private async getBytesUsed(): Promise<number> {
        const stmt = this.db.prepare("SELECT COALESCE(SUM(LENGTH(data)), 0) as total FROM fs_data")
        const row = await stmt.get() as { total: number }

        return row.total
    }

    private async assertHasSpace(bytesToWrite: number): Promise<void> {
        if (bytesToWrite <= 0) return;

        const max = await this.getMaxBytes();

        if (max === null) return;

        const used = await this.getBytesUsed();

        if (used + bytesToWrite > max) {
            throw createFsError({
                code: 'ENOSPC',
                syscall: 'write',
                path: '',
    ...
    // we can add our check for the available disk size on host device
    async writeFile(
        path: string,
    ...
    private async updateFileContent(
        ...
        const row = await sizeStmt.get(ino) as { size: number } | undefined;

        const currentSize = row?.size ?? 0;
        const difference = buffer.length - currentSize;

        if (difference > 0) {
            await this.assertHasSpace(difference);
        }
    ...
    async copyFile(src: string, dest: string): Promise<void> {
        ...
        await this.assertHasSpace(srcRow.size);
    ...
    async statfs(): Promise<FilesystemStats> {
        const inodeStmt = this.db.prepare(
